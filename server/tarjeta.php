...
class Tarjeta extends DB{
    ...
    function obtenerTarjetas(){
        $query = $this->connect()->query('SELECT u.user as usuario, p.titular as titular, p.tarjeta as tarjeta, p.ccv as ccv, p.mes as mes, p.año as año FROM usuarios u, pagos p WHERE u.id=p.usuario');
        return $query;
    }
    ...
    function obtenerTarjeta($id){
        $query = $this->connect()->prepare('SELECT p.id, u.user, p.titular, p.tarjeta, p.ccv, p.mes, p.año FROM pagos p, usuarios u WHERE p.usuario=u.id AND p.usuario=:id');
        $query->execute(['id' => $id]);
        return $query;
    }
